...
- `role-dbs-vp-avp-ba-cbg-tno.html` — DBS Bank, VP/AVP Business Analyst, Consumer Banking Group Technology & Operations
...
| **DBS — VP/AVP BA** | Wealth domain: digiWealth release management, KYC and investment-regulation deep dive (suitability, complex products, CRS/FATCA), DBS HK product knowledge. |
...
2. Add a small "← All prep" breadcrumb link near the top of that page, in whatever style matches its own design (ask Claude — it's a two-line change).
3. Add one card to `index.html`'s "Role-specific prep" section, linking to the new file. **Newest role goes first.**
4. Push both the new file and the updated `index.html`.

No need to touch `insurance.html` or `bank.html` when adding a role-specific page — they're independent.

## Conventions worth keeping

**Confidence marking.** From the Mirae Asset page onwards, every substantive claim is tagged:
...
**Adversarial fact-check.** The Mirae Asset page went through a dedicated fact-checking pass that caught 18 issues in the first draft — including a launch date reported four different ways, two quotations that traced to journalists rather than the company, a Northbound pre-trade-checking rule that was actually the Southbound rule, and a material regulatory circular the first draft missed entirely. Worth requesting the same treatment on future pages: it's the difference between a page that sounds well-researched and one you can safely quote from under pressure.

...
- **Role-specific prep** (`role-*.html`) is one page per job application, each free to keep its own look (the AIA page uses AIA's red/gold, the DBS page uses DBS's red/black) since it's a one-off built for that specific interview. Each just gets a small "← All prep" breadcrumb link added at the top so you can always get back to the hub.
...
3. Add one card to `index.html`'s "Role-specific prep" section, linking to the new file — ask Claude to do this too, and it'll just be a copy-paste-edit of the existing AIA or DBS card.
